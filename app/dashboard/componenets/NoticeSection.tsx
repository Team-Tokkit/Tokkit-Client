...
export default function NoticesSection({ notices, currentNotice, onNoticeChange }: NoticesSectionProps) {
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                <AnimatePresence mode="wait">
                    {notices.map((notice, index) => (
                        <motion.div
                            key={notice.id}
                            className={`cursor-pointer ${index === currentNotice ? "block" : "hidden"}`}
                            ...
                            <div className="p-4">
                                <div className="flex items-center justify-between mb-2">
                                    <div className="flex items-center">
                                        {notice.isNew && <span className="inline-block w-2 h-2 bg-red-500 rounded-full mr-2"></span>}
                                        <h4 className="font-medium text-[#111827] text-sm">{notice.title}</h4>
                                        {notice.isEvent && (
                                            <span className="ml-2 text-[10px] px-2 py-0.5 bg-blue-100 text-blue-800 rounded-full">이벤트</span>
                                        )}
                                    </div>
                                    <span className="text-xs text-[#6B7280]">{notice.date}</span>
                                </div>
                                <p className="text-xs text-[#6B7280] line-clamp-2">{notice.content}</p>
                            </div>
                        </motion.div>
                    ))}
                </AnimatePresence>
