...
export default function AddressSearchModal({ onClose, onSelect }: AddressSearchModalProps) {
                        ...
                        geoResult.forEach((item: any) => {
                            if (!prev.some((p) => p.address_name === item.address_name)) {
                                combined.push({
                                    ...
                                    road_address_name: item.road_address?.address_name || item.address_name,
                                })
                            }
                        })
                        return combined
    ...
    const handleSelect = (item: any) => {
        ...
        onSelect({ businessAddress: address, latitude: lat, longitude: lng, zipcode: zip, sido, sigungu })
        onClose()
    }
            ...
            <div className="flex-1 bg-[#F5F5F5] p-4 overflow-y-auto">
                <div className="bg-white rounded-xl p-4 mb-4">
                    <div className="flex gap-2">
                        ...
                        />
                        <Button onClick={searchAddress} disabled={searchingAddress || !addressKeyword.trim()} className="h-12 px-4">
                            {searchingAddress ? <RefreshCw className="h-4 w-4 mr-2 animate-spin" /> : <Search className="h-4 w-4 mr-2" />}
                            검색
                        </Button>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-xl overflow-hidden">
                        <ul className="divide-y divide-gray-200">
                            {addressResults.map((item, index) => (
                                <li key={index} className="p-4 hover:bg-gray-100 cursor-pointer" onClick={() => handleSelect(item)}>
                                    <div className="flex items-start">
                                        <Building className="h-5 w-5 text-gray-400 mt-0.5 mr-2" />
                                        <div>
                                            <p className="text-sm font-medium text-gray-800">{item.place_name !== item.address_name ? item.place_name : "주소"}</p>
                                            <p className="text-xs text-gray-600">{item.road_address_name || item.address_name}</p>
                                            {item.road_address_name && item.road_address_name !== item.address_name && (
                                                <p className="text-xs text-gray-400">{item.address_name}</p>
                                            )}
                                        </div>
                                    </div>
                                </li>
